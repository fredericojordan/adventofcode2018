...
defmodule Puzzle12 do
  ...
  defp read_plants_file() do
    {:ok, file_content} = File.read("input12.txt")

    [first_line, _ | combinations] = String.split(file_content, "\n")
    ...
    {initial_state, combination_map}
  end


  def solve do
    {initial_state, combination_map} = read_plants_file()

    {final_state, zero_position} =
      Stream.iterate(initial_state, fn x ->
        '...' ++ x ++ '...'
        |> Enum.chunk_every(5, 1)
        |> Enum.map(fn c -> Map.get(combination_map, c, '.') end)
    ...
    final_state
    |> Stream.with_index()
    |> Stream.filter(fn {pot, _index} -> [pot] == '#' end)
    |> Stream.map(fn {_pot, index} -> index-zero_position end)
    |> Enum.sum()
  end
end
