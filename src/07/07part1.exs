...
defmodule Puzzle07 do
  ...
  or not, this box contains something that will solve all of Santa's transportation problems - at least, that's what it
  looks like from the pictures in the instructions." It doesn't seem like they can read whatever language it's in, but
  you can: "Sleigh kit. Some assembly required."
  ...
  defp get_availables(tasks, dependencies, sent) do
    tasks
  ...
  def solve do
    ...
    free_tasks =
      tasks
      |> Enum.filter(fn x -> !Enum.member?(Map.keys(dependencies), x) end)
      |> Enum.sort()

    {[], free_tasks, {tasks, dependencies}}
